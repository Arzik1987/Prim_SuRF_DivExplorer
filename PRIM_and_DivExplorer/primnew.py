...
class PRIM:
    def __init__(self, alpha=0.05, patience=0, bslevel=None, fprfnr=False, mass_min=0.1):
        self.alpha = alpha  # peeling parameter
        self.patience = patience  # extra parameter to control patience
        self.bslevel = bslevel
    ...
    def _target_fun_fprfnr(self, npos, n):
        bsl = self.Np_ / self.Nnotna_ if self.bslevel is None else self.bslevel
        tar = npos / n - bsl
        return tar
    ...
    def _box_quality(self, in_box):
        ...
        if n_in_box/self.N_ > self.mass_min_:
            # Calculate quality
            if self.fprfnr_:
                n_pos_in_box = np.count_nonzero(self.y_[in_box] == 1)
                notna_in_box = self.y_[in_box].notna().sum()
                quality = self._target_fun_fprfnr(n_pos_in_box, notna_in_box) * (n_in_box ** self.patience)
            else:
    ...
    def _set_fit_vars(self):
        self.N_ = len(self.y_)
        self.Nnotna_ = self.y_.notna().sum()
    ...
    def get_size(self):
        return [bsize/self.N_ for bsize in self.sizes_]
